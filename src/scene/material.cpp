...
vec3f Material::shade( Scene *scene, const ray& r, const isect& i ) const
{
	...
	vec3f result = ke + prod(ka, getAmibientLightsIntensity(scene));
	...
	for (Light* light : scene->getLights()) {
		...
		if (!(u.dot(i.N) <= 0.0 || shadowAtten.iszero())) {
			...
			const vec3f& specular = ks * pow(
				std::max<double>(0.0, refl_dir.dot(-r.getDirection())),
				shininess * 128
			);
			result += prod(prod(atten, intensity), diffuse + specular);
		}
	}
